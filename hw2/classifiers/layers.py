    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            for f in range(F):
                ...
                dx_pad[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW] += w[f] * (dout[:, f, i, j])[:, None, None, None]
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    x_slice = x[n, c, i*stride:i*stride+HH, j*stride:j*stride+WW]
                    max_val = np.max(x_slice)
                    dx[n, c, i*stride:i*stride+HH, j*stride:j*stride+WW] += (x_slice == max_val) * dout[n, c, i, j]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
